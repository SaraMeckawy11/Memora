...
export default function CaptionSection() {
  ...
  const handleFontSelect = (fontName) => {
    store.updateCurrentPageSettings({ textStyle: { ...currentPage?.textStyle, fontFamily: fontName } });
    setFontSelectOpen(false);
  };
  ...
  const selectedFontFamily = currentPage?.textStyle?.fontFamily || store.selectedFontFamily;
  const selectedFontSize = currentPage?.textStyle?.fontSize || store.selectedFontSize;
  const selectedFontColor = currentPage?.textStyle?.color || store.selectedFontColor;
  const captionPosition = currentPage?.textStyle?.position || store.captionPosition;
  const captionAlignment = currentPage?.textStyle?.alignment || store.captionAlignment;
  const selectedCaption = currentPage?.textContent || '';
